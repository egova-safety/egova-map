...
export const CLUSTER_LAYER_OPTIONS: any = {
 ...
 * 点聚合图层
 */
export class ClusterLayer extends base.BusinessLayer {
    private dataManager: DataManager;

    public constructor(mapView: base.MapView, id: string, options: any) {
        super(mapView, id, { ...CLUSTER_LAYER_OPTIONS, ...options });
    ...
    public createClusterLayer(options: any) {
        ...
        let layer = new esri.layers.ClusterLayer({
            id: this.id,
            spatialReference: this.mapView.spatial,
        ...
        let defaultSym = new esri.symbol.SimpleMarkerSymbol()
            .setSize(6)
            .setColor(options.font.color)
            .setOutline(options.font.outline);
        let renderer = new esri.renderer.ClassBreaksRenderer(
            defaultSym,
            "clusterCount"
        );
        let xlSymbol = new esri.symbol.SimpleMarkerSymbol(
            esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,
            options.levels.xl.size,
            new esri.symbol.SimpleLineSymbol(
                esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                new dojo.Color(options.levels.xl.color.line),
                1
            ),
            new dojo.Color(options.levels.xl.color.fill)
        );
        let lgSymbol = new esri.symbol.SimpleMarkerSymbol(
            esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,
            options.levels.lg.size,
            new esri.symbol.SimpleLineSymbol(
                esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                new dojo.Color(options.levels.lg.color.line),
                1
            ),
            new dojo.Color(options.levels.lg.color.fill)
        );
        let mdSymbol = new esri.symbol.SimpleMarkerSymbol(
            esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,
            options.levels.md.size,
            new esri.symbol.SimpleLineSymbol(
                esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                new dojo.Color(options.levels.md.color.line),
                1
            ),
            new dojo.Color(options.levels.md.color.fill)
        );
        let smSymbol = new esri.symbol.SimpleMarkerSymbol(
            esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE,
            options.levels.sm.size,
            new esri.symbol.SimpleLineSymbol(
                esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                new dojo.Color(options.levels.sm.color.line),
                1
            ),
            new dojo.Color(options.levels.sm.color.fill)
        );
        renderer.addBreak(
            options.levels.sm.start,
            options.levels.sm.end,
            smSymbol
        );
        renderer.addBreak(
            options.levels.md.start,
            options.levels.md.end,
            mdSymbol
        );
        renderer.addBreak(
            options.levels.lg.start,
            options.levels.lg.end,
            lgSymbol
        );
        renderer.addBreak(
            options.levels.xl.start,
            options.levels.xl.end,
            xlSymbol
        );
        ...
        if (options.areaDisplay) {
            ...
            let defaultAreaSym = new esri.symbol.SimpleFillSymbol()
                .setStyle(esri.symbol.SimpleFillSymbol.STYLE_SOLID)
                .setColor(new dojo.Color([0, 0, 0, 0.2]))
                .setOutline(
                    new esri.symbol.SimpleLineSymbol(
                        esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                        new dojo.Color([0, 0, 0, 0.3]),
                        1
                    )
                );
            let areaRenderer = new esri.renderer.ClassBreaksRenderer(
                defaultAreaSym,
                "clusterCount"
            );
            let xlAreaSymbol = new esri.symbol.SimpleFillSymbol(
            ...
            let lgAreaSymbol = new esri.symbol.SimpleFillSymbol(
                esri.symbol.SimpleFillSymbol.STYLE_SOLID,
                new esri.symbol.SimpleLineSymbol(
                    esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                    new dojo.Color(options.levels.lg.color.line),
            ...
            let mdAreaSymbol = new esri.symbol.SimpleFillSymbol(
                esri.symbol.SimpleFillSymbol.STYLE_SOLID,
                new esri.symbol.SimpleLineSymbol(
                    esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                    new dojo.Color(options.levels.md.color.line),
            ...
            let smAreaSymbol = new esri.symbol.SimpleFillSymbol(
                esri.symbol.SimpleFillSymbol.STYLE_SOLID,
                new esri.symbol.SimpleLineSymbol(
                    esri.symbol.SimpleLineSymbol.STYLE_SOLID,
                    new dojo.Color(options.levels.sm.color.line),
    ...
    public getPreClusteredGraphics() {
        ...
        let clusteredData = this.dataManager.clientSideClustering(
            clusterRatio,
            maxSingleFlareCount,
            clusterOptions.areaDisplay,
            this.mapView.map
        );
    ...
    public getImageUrl(item: any): string {
        ...
        if (typeof imageUrl === "string" && imageUrl) {
            const key = `imageUrl${item.status || ""}${
                item.selected ? "checked" : ""
            }`;
            let statusImageUrl: string =
                (<any>this.options)[key] ||
            ...
            const key = "image" + status + (item.selected ? "checked" : "");
            return (
                (<any>this.options)[key] ||
                this.options.symbol[key] ||
    ...
    public get graphics(): Array<base.Graphic> {
        if (this.enableCluster) {
    ...
    public onUpdateGraphicByModel(item: any): void {
        ...
        graphic.draw(); // 重绘
        if (!base.MapUtils.isEqualPoint(pt, originPoint)) {
            this.options.onPositionChanged(pt, originPoint, graphic.attributes);
        }
    }

    protected add(graphic: base.Graphic) {
        if (this.enableCluster) {
    ...
    protected remove(graphic: base.Graphic) {
        if (this.enableCluster) {

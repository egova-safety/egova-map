...
export class MapUtils {
    ...
    public static isEqualPoint(originGeometry: any, targetGeometry: any) {
        ...
        return (
            originGeometry.x === targetGeometry.x &&
            originGeometry.y === targetGeometry.y
        );
    }
    ...
    public static delta(lat: any, lon: any) {
        ...
        dLat =
            (dLat * 180.0) /
            (((a * (1 - ee)) / (magic * sqrtMagic)) * MapUtils.PI);
        dLon =
            (dLon * 180.0) / ((a / sqrtMagic) * Math.cos(radLat) * MapUtils.PI);
    ...
    public static bd_encrypt(gcjLat: number, gcjLon: number) {
        let x = gcjLon,
            y = gcjLat;
        let z =
            Math.sqrt(x * x + y * y) + 0.00002 * Math.sin(y * MapUtils.X_PI);
    ...
    public static mercator_encrypt(wgsLat: number, wgsLon: number) {
        ...
        let y =
            Math.log(Math.tan(((90 + wgsLat) * MapUtils.PI) / 360)) /
            (MapUtils.PI / 180);
        y = (y * 20037508.34) / 180;
        return { lat: y, lon: x };
    }
    ...
    public static mercator_decrypt(mercatorLat: number, mercatorLon: number) {
        ...
        y =
            (180 / MapUtils.PI) *
            (2 * Math.atan(Math.exp((y * MapUtils.PI) / 180)) -
                MapUtils.PI / 2);
        return { lat: y, lon: x };
    }
    /**
    ...
    public static transformLat(x: number, y: number) {
        ...
        ret +=
            ((20.0 * Math.sin(6.0 * x * MapUtils.PI) +
                20.0 * Math.sin(2.0 * x * MapUtils.PI)) *
                2.0) /
            3.0;
        ret +=
            ((20.0 * Math.sin(y * MapUtils.PI) +
                40.0 * Math.sin((y / 3.0) * MapUtils.PI)) *
                2.0) /
            3.0;
        ret +=
            ((160.0 * Math.sin((y / 12.0) * MapUtils.PI) +
                320 * Math.sin((y * MapUtils.PI) / 30.0)) *
                2.0) /
            3.0;
        return ret;
    }
    public static transformLon(x: number, y: number) {
        ...
        ret +=
            ...
            3.0;
        ret +=
            ((20.0 * Math.sin(x * MapUtils.PI) +
                40.0 * Math.sin((x / 3.0) * MapUtils.PI)) *
                2.0) /
            3.0;
        ret +=
            ((150.0 * Math.sin((x / 12.0) * MapUtils.PI) +
                300.0 * Math.sin((x / 30.0) * MapUtils.PI)) *
                2.0) /
            3.0;
        return ret;
    }

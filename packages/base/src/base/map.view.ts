...
import { IMapSetting } from "./map.setting";

import { MapUtils } from "./map.utils";

import { Point, InfoWindowShowEventArgs, Graphic } from "./map.model";
import { EventArgs, EventProvider } from "../events";
...
export abstract class MapView extends EventProvider {
    public featureLayers: Array<FeatureLayer> = [];
    public baseLayers: Array<TiledLayer> = [];
    public options: any;
    public spatial: any;
    public innerMap: any;
    ...
    public onFormPoint(point: any): { latitude: number; longitude: number } {
        ...
        if (this.spatial.wkid !== this.mapSetting.wkidFromApp) {
            if (
                this.spatial.wkid === 3857 &&
                this.mapSetting.wkidFromApp === 4326
            ) {
                lnglat = MapUtils.mercator2lonlat(lnglat.lat, lnglat.lon);
            } else if (
                this.spatial.wkid === 102100 &&
                this.mapSetting.wkidFromApp === 4326
            ) {
                lnglat = MapUtils.mercator_decrypt(lnglat.lat, lnglat.lon);
            } else if (
                this.spatial.wkid === 4326 &&
                this.mapSetting.wkidFromApp === 3857
            ) {
                lnglat = MapUtils.mercator_decrypt(lnglat.lat, lnglat.lon);
            } else if (
                this.spatial.wkid === 4326 &&
                this.mapSetting.wkidFromApp === 3889
            ) {
     ...
     * @param item 要素原型
     */
    public onToPoint(item: any): Point {
        let lnglat = {
        ...
        if (this.spatial.wkid !== this.mapSetting.wkidFromApp) {
            if (
                ...
                this.spatial.wkid === 4326 &&
                this.mapSetting.wkidFromApp === 3589
            ) {
                lnglat = MapUtils.gcj_decrypt_exact(lnglat.lat, lnglat.lon);
            } else {
    ...
    public centerAt(): Promise<void> {
        ...
        switch (args.length) {
            case 0:
                if (
                    this.mapSetting.center &&
                    this.mapSetting.center.length === 2
                ) {
                    pt = this.getPoint({
     ...
     * @param id 底图id
     */
    public getBaseLayerById(id: string): TiledLayer | null {
     ...
     * @param id
     */
    public getFeatureLayerById(id: string): FeatureLayer | null {
     ...
     * @param featureLayer
     */
    public addFeatureLayer(featureLayer: FeatureLayer): void {
     ...
     * @param point 点
     */
    public abstract onCenterAt(point: Point): Promise<void>;
     ...
     * @param point 点
     */
    public abstract onCreatePoint(item: any): Point;
     ...
     * 创建底图
     */
    public abstract onCreateBaseLayers(): Array<TiledLayer>;

    /**
     * 显示要素tooltip信息
     * @param graphic 要显示tootip信息的要素
     */
    public abstract onShowTooltip(graphic: Graphic): void;

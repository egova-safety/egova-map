...
import { TrackLine } from "./map.model";
...
export class TrackLayer implements ICombineLayer {
    ...
    public constructor(
         ...
         * 当轨迹上的移动物体进出卡口点事件回调
         */
        routeLayer.options.onStationEvent = (
            name: string,
            index: number,
            graphic: any,
            ...
            if (index === 0 && isStartPoint) {
                this.businessLayer.saveGraphicList([
                    graphic.attributes.__model
                ]);
                this.options.onStationChanged(
                    index,
                    graphic.attributes.__model
                );
            }
        ...
        routeLayer.options.onMoveEvent = (
            lineName: string,
            segmentIndex: number,
            xy: Array<any>,
            angle: number
        ...
        routeLayer.options.onLineEndEvent = (
            lineName: string,
            segmentIndex: number,
            trackLine: TrackLine
        ) => {
    ...
    public showTrackToolBox(): void {
        ...
        if (this.mapView.innerMap.container) {
            this.mapView.innerMap.container.appendChild(this._trackToolBox);
        } else {
            document
                .querySelector("#" + this.mapView.mapElement)
                .appendChild(this._trackToolBox);
        }

        this._playButton = document.querySelector(
            "#" + this.toolBoxId + " .continue"
        );
        this._pauseButton = document.querySelector(
            "#" + this.toolBoxId + " .pause"
        );
        this._speedUpButton = document.querySelector(
            "#" + this.toolBoxId + " .up"
        );
        this._speedDownButton = document.querySelector(
            "#" + this.toolBoxId + " .down"
        );
        this._clearButton = document.querySelector(
            "#" + this.toolBoxId + " .clear"
        );
        this._toolBoxText = document.querySelector(
            "#" + this.toolBoxId + " .route-text"
        );

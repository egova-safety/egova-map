 ...
 * 业务图层
 */
export abstract class BusinessLayer extends FeatureLayer {
     ...
     */
    public creatGraphicByModel(item: any): Graphic {
        item = this.onChangeStandardModel(item);
     ...
     * 修改要素
     */
    protected updateGraphicByModel(
        item: any,
        graphic: any | null = null
    ): void {
        item = this.onChangeStandardModel(item);
        if (!this.onValidModel(item)) {
            return;
    ...
    public clearSelectStatus(): void {
        ...
        for (let i = 0; i < graphics.length; i++) {
            if (
                graphics[i].attributes.selected ||
                typeof graphics[i].attributes.selected !== "boolean"
            ) {
     ...
     */
    public setSelectStatusByModels(
        dataList: Array<any>,
        refresh: boolean
    ): void {
        if (refresh) {
     ...
     * 获取所有选中的要素
     */
    public getSelectedGraphics(): Array<Graphic> {
    ...
    public gotoCenterById(id: string): void {
        ...
        this.mapView.centerAt(pt.x, pt.y);
    }
    ...
    public removeFromMap(): void {
        this.mapView.removeFeatureLayer(this.id);
    }
     ...
     * @param item
     */
    protected abstract onCreatGraphicByModel(item: any): Graphic;

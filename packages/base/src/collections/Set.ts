import { ISet, IEnumerator, Enumerator, IEnumerable } from ".";
...
export class Set<T> implements ISet<T> {
     ...
     */
    public forEach(
        callback: (item: T, index: number, set: Set<T>) => void,
        scope?: any
    ): void;
     ...
     */
    public forEach(
        callback: (value: T, source: IEnumerable<T>) => void,
        scope?: any
    ): void;
    ...
    public forEach(): void {
        ...
        for (let i = 0, len = values.length; i < len; i++) {
            fromEnumerable
                ? callback.call(scope, values[i], this)
     ...
     */
    public find(
        callback: (value: T, index: number, set: Set<T>) => boolean,
        scope?: any
    ): T {
        let values = this._values;

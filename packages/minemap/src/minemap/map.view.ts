...
import { Point, MapSpatial } from "./map.model";
...
export class MapView extends base.MapView {
     ...
     * @param options 点属性
     */
    public onCreatePoint(options: any): Point {
        return new Point(
            options.x,
            options.y,
    ...
    public onCreateMap() {
        ...
        minemap.spriteUrl =
            "http://" +
            this.mapSetting.mapDomain +
            "/minemapapi/" +
            this.mapSetting.mapVersion +
            "/sprite/sprite";
        minemap.serviceUrl = "http://" + this.mapSetting.mapDomain + "/service";
        minemap.accessToken =
            this.mapSetting.accessToken || "25cc55a69ea7422182d00d6b7c0ffa93";
        ...
        this.spatial = new MapSpatial(minemap.solution);
    ...
    public onShowInfoWindow(evt: base.InfoWindowShowEventArgs): void {
        ...
        if (evt.options) {
            ...
            this.innerMap.infoWindow = new minemap.Popup({
                closeOnClick: false,
                closeButton: true,
                offset: [0, -14]
            });
        ...
        if (evt.context) {
            switch (evt.context.type) {
                case "dom":
                    this.innerMap.infoWindow.setDOMContent(
                        document.getElementById(evt.context.content) || ""
                    );
        ...
        this.innerMap.infoWindow.setLngLat([
    ...
    public onShowTooltip(graphic: any): void {
        ...
        if (this.tooltipElement === undefined) {
            throw new base.Exception("tooltipElement is null");
        }

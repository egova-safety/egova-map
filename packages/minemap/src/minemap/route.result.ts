import { Polyline, Point } from "./map.model";
/**
 * 思维图新路由结果定义
 */
export class RouteResult {
    ...
    public data: { error: string | undefined; rows: Array<RouteRow> | undefined };
    ...
    public constructor(result: any) {
        ...
        if (result.data) {
            this.data = { error: undefined, rows: undefined };
    ...
    public getLine(spatial: any) {
        let polyline = new Polyline(spatial);
...
export class RouteRow {
    ...
    public getPoints(spatial: any): Array<Point> {
        let mps: Array<Point> = [];
        this.points.forEach(g => {
            mps.push(new Point(g[0], g[1], spatial));
        });
        return mps;
    }
